...
const Home = () => {
	...
	useEffect(() => {
		const loadMatches = async () => {
			try {
				const querySnapshot = await getDocs(collection(db, 'savedMatches'));
				const matchCards = querySnapshot.docs.map((doc) => {
					const data = doc.data();
					return {
						id: doc.id,
						name: data.name || 'Unknown Destination',
						tripPic: data.image || destinations,
						onClick: () => navigate(`/destination/${doc.id}`),
					};
				});
